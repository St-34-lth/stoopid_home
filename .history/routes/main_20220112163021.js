...
function makeQuery(deviceName,data)
{
          ...
          {
                    query = `insert into devices(name,condi,status,volume) values('${deviceName}','${data.deviceCondition}',${data.deviceStatus},${data.deviceVol})`
          }
          else if(deviceProps.deviceProperties.includes('temperature'))
          {
                    query = `insert into devices(name,condi,status,temperature) values('${deviceName}','${data.deviceCondition}',${data.deviceStatus},${data.deviceTemp});`
          }
          else 
          {
                    query `insert into devices(name,condi,status) values('${deviceName}','${ddata.eviceCondition}','${data.deviceStatus}');`
          }
          return query 
}
...
homeAdmin.post('/registerDevice', async (req,res,next) => 
{
          ...
          decider(req.body.deviceName,req.body)

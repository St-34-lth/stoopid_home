...
document.getElementById('accordionBase').addEventListener('devicesLoaded', getDeviceListings)

function getdeviceListings(e)
{
          e.optionAssets= JSON.parse(optionAssets)
          e.deviceAssets = JSON.parse(deviceAssets)

          deviceListingsCollection = document.getElementsByTagName('img')
          console.log(e)
          // for(i in deviceListingsCollection)
          // {
          //           let imgElmt= deviceListingsCollection[i]
          //           let propertyID= imgElmt.id.split('Image',1)
          //           console.log(imgElmt,propertyID)
          //           switch(propertyID)
          //           {
                              
          //                     case "device":
          //                               assingDeviceAssets(imgElmt.name,deviceAssets)
          //                               break;
          //                     case "status":
          //                               assignOptionAssets(propertyID,optionAssets)
          //                               break;
...
function assingDeviceAssets(deviceName,deviceAssets)
{
          
          document.getElementById(`${deviceName}`+'Image').src = optionAssets[deviceProperty]
}
...
function eventHandler(event,optionAssets,deviceAssets)
{
          ...
          if(event.type='DOMContentLoaded')
          {
                    ...
                    if(event.path[0].URL.includes('devices'))
                    {
                              document.getElementById('accordionBase').dispatchEvent('devicesLoaded')
                              // getdeviceListings(optionAssets)
                    }
          }

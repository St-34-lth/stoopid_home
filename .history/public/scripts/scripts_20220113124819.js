function optionDisabler(selection,elmts,availableDevices)
{
          ...
          let extraProps = ["temperature","volume"]
          availableDevices= JSON.parse(availableDevices)

          let deviceProperties = getDeviceProps(selection,availableDevices,basicProps,extraProps)
...
{
          let basicProperties = basicProps.length > 0 ? basicProps : ["status","condition"] 
          let extraProperties = extraProps.length > 0 ? extraProps : ["temperature","volume"] 
...
//dialogue window event listener. Displays a confirm dialogue and submits the form if yes, cancels otherwise.

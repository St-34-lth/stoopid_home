...
function validateUpdateForm(availableDevices,formID)
{
          ...
          for(d of deviceProps.deviceProperties)
          {
                    ...
                    if (deviceProperty =='tempearture')
                    {
                              checkPropertyValues(deviceProperty,deviceRestrictions,deviceProperty.value)
                    }
                    // check restrictions here 
                    if(!validatePropertyValueTypes(deviceProperty.name,deviceProperty.value))
                    {
...
function checkPropertyValues(property,deviceRestrictions,value)
{
          ...
          { 
                    let min = deviceRestrictions[0]
                    let max = deviceRestrictions[1]
          ...
          //           //i guess i can refactor this ?
                    ...
                    //           let disabledProperties = availableProperties.filter(item=> !deviceProperties.includes(item))
                              ...
                              for (i in deviceProperties.deviceProperties)
                              {
                                        ...
                                        if(!validatePropertyValueTypes(deviceProperty,value))
                                        {
